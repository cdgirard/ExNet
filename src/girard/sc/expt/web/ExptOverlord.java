 ...
 * 
 * TODO: Need to change all java.util classes to use generics TODO: Need to
 * switch system to a newer GUI management system.
 * 
 * <p>
 * <br>
 * Started: 2001 <br>
...
public class ExptOverlord extends WLOverlord
{
    ...
    /**
     * Add a WebPanel to be displayed within the m_WebpageBasePanel. Overridden
     * so that the m_TitleCanvas says ExNet 3.0 rather than Web-Lab.
     *
     * @param A
     *            The WebPanel to be displayed.
     */
     ...
     * <p>
     * fileInfo should have the following format: <br>
     * Key: "FileName" Data: A String that is the Name_VC for the BaseAction
     * file. <br>
     * (Optional) Key: "App ID" Data: A String that is the App_ID value for the
     * file.
     *
     * @param db
     *            The database that the BaseAction files are located in.
     * @param dbTable
     *            The table where the BaseAction files are located.
     * @param fileInfo
     *            Key information about the file so it can be located withing
     ...
     * @see java.net.URL
     */
    public String getHelpLoc(String str)
    {
	String helpStr = null;

	helpStr = HELP_DIR + str;
    ...
    /**
     * Calls up a HelpWindow if one hasn't already been created. In addition it
     * also loads the help file specified by the String passed in.
     *
     * @param key
     *            The specific help file to display when the HelpWindow is
     *            created.
     * @see girard.sc.expt.help.HelpWindow
     */
    ...
    /**
     * Requests a list of Access Groups that can be accessed by the user for
     * granting or determining access rights to a file. Does this by sending a
     * WLAccessGroupListReqMsg. The Vector returned contains a list of
     * Hashtables, each Hashtable has the following format: <br>
     * App_ID stored as a String under the key "App ID" <br>
     * App_Name_VC stored as a String under the key "App Name" <br>
     * App_Desc_VC stored as a String under the key "App Desc"
     * 
     * @return Returns the list of access groups in the form of a Vector of
     *         Hashtables.
    ...
    /**
     * Loads the choosen filename by using the LoadBaseActionRegMsg class. The
     * fileInfo Hashtable should contain the same info as the Hashtables
     * retrieved by loadBaseActionFileList.
     *
     * @param fileInfo
     *            The Hashtable of information on the BaseAction file to be
     *            loaded.
     * @param ba
     *            The actual BaseAction class object type that is being loaded.
     * @return Returns the loaded BaseAction.
     * @see girard.sc.expt.io.msg.LoadBaseActionReqMsg
     * @see girard.sc.expt.web.ExptOverlord#loadBaseActionFileList(String db,
     *      String dbTable, Vector ag)
     */
     ...
     * user. The Hashtable contains the following information: <br>
     * The filename of the saved file. (Key: "FileName", Object: String) <br>
     * The description of the saved file. (Key: "Desc", Object: String) <br>
     * (The following two objects may be null and thus not in the Hashtable.) <br>
     * The access group id associated with the file. (Key: "App ID", Object:
     * String) <br>
     * The access group name associated with the file. (Key: "App Name", Object:
     * String)
     *
     * @param db
     *            The database where the BaseAction files are stored.
     * @param dbTable
     *            The database table where the files are stored.
     * @param ag
     *            The Vector of access group Hashtable information.
    ...
    /**
     * Requests a list of saved Experiment files that can be loaded by the user,
     * based on user's ID and user's group access. Does this by sending a
     * ExptFileListReqMsg. The Vector of Experiments returns only contains the
     * base information about each Experiment. All the information for a
     * specific Experiment can be retrieved using loadExpt(Experiment expt).
     * 
     * @param ag
     *            The Vector of access groups.
     ...
     *
     * @param fileName
     *            The value of the Sim_Name_VC for the SimActor.
     * @param sa
     *            The SimActor class type that the file is.
     * @param ag
     *            The Hashtable of the access group related to this SimActor.
    ...
    /**
     * Retrieves a list of SimActors of a given type that the user has access
     * to. The Vector returns a list of Hashtables each with the following
     * format: <br>
     * Key: "Sim Name" Data: A String containing the value for Sim_Name_VC <br>
     * Key: "Sim Desc" Data: A String containing the value for Sim_Desc_VC <br>
     * (Optional)Key: "App ID" Data: A String containing the value for App_ID
     * 
     * @param sa
     *            The SimActor type.
     * @param ag
     *            The Vector of access groups that the user has access to.
    ...
    /**
     * Removes a WebPanel from being dipslayed in m_WebpageBasePanel and sets
     * m_activePanel to null. Overridden so that the label of the m_TitleCanvas
     * is set to ExNet 3.0 instead of Web-Lab.
     *
     * @param R
     *            The WebPanel to be removed.
     */
     ...
     *
     * @param ba
     *            The BaseAction to be saved.
     * @param ag
     *            The access group to attach it to.
    ...
    /**
     * Sends an ExptMessage to the WLGeneralServer. Any ExptMessage response is
     * returned.
     *
     * @param em
     *            The ExptMessage to be sent.
     * @return Returns the ExptMessage response or null if there was no
     *         response.
	...
	if (nos.getFlag())
	{
	    ...
	    {
		System.err.println("ok...here it is");
