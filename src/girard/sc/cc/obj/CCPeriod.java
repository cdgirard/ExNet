...
public class CCPeriod extends NetworkComponent 
    {
    ...
    public CCPeriod (Hashtable nodes)
        {
    ...
    public CCPeriod (int p, int r, int t, Hashtable nodes)
        {
        m_period = p;
        m_rounds = r;
        m_time = t;
    ...
    public CCPeriod (int p, int r, int t, int[] nodes)
        {
        m_period = p;
        m_rounds = r;
        m_time = t;
    ...
    public Object clone()
        {
        CCPeriod ccp = new CCPeriod(m_period,m_rounds,m_time,m_userOrder);

        ccp.setCurrentRound(m_currentRound);
        ccp.setCurrentTime(m_currentTime);
  ...
  // Draw info on a canvas using the graphic for the experimenter screen.
    ...
    public CCStateAction getNextState(CCNetwork ccn)
        ...
        if (currentState < START_ROUND_STATE)
            {
            return new CCRoundWindowStateAction(this);
            }
        if (currentState < END_ROUND_STATE)
            {
            return new CCEndRoundStateAction();
            }
        return null;
        }
