...
/* 
   Used to display offers being sent within a CCNetwork.
...
public class CCEdgeDisplay extends CCNetworkComponent 
    {
    ...
    protected CCEdge      m_edge = null; // Which edge the display is for.
/* index 0 is keep, index 1 is give, index 2 is type */
  ...
  /** From Node Stuff **/
    ...
    public CCEdgeDisplay()
        {
        super("CCEdgeDisplay");
        }
    public CCEdgeDisplay (CCEdge edge)
        {
        super("CCEdgeDisplay");
        m_edge = edge;
        }
    public CCEdgeDisplay (CCEdge edge,CCNetwork net)
        {
        super(net,"CCEdgeDisplay");
        m_edge = edge;
        }
...
/* Do we want/need to clone all the objects attached to this object...possibly so */
        ...
        CCEdgeDisplay cced = new CCEdgeDisplay();
        for (int i=0;i<3;i++)
            {
            cced.setN1Display(i,m_n1Display[i]);
            cced.setN2Display(i,m_n1Display[i]);
            }

        cced.setExchangeState1(m_exchangeState1);
        cced.setExchangeState2(m_exchangeState2);
...
// May not need this guy...
    public Hashtable getSettings()
        ...
        settings.put("Type","DB-CCEdgeDisplay");
...
/* Returns the proper color to display a given state in */
    public Color getStateColor(int state)
        {
        if (state == RED)
            return CCColor.edgeRed;
        if (state == YELLOW)
            return CCColor.edgeYellow;
        if (state == GREEN)
            return CCColor.edgeGreen;
        if (state == COMPLETED)
            return CCColor.edgeGreen;

        return CCColor.edgeBlack;
        }
  ...
  // Reset object data to starting values.
    ...
    public static void updateDBEntry(Connection con)
        ...
        try
            ...
            ResultSet rs = stmt.executeQuery("SELECT Object_ID_INT FROM Other_Objects_T WHERE Object_Name_VC = 'DB-CCEdgeDisplay'");
            ...
            if (rs.next())
                ...
                CCEdgeDisplay ed = new CCEdgeDisplay();
...
System.err.println(ed);
                ...
                cs.setString(1,"DB-CCEdgeDisplay");
                cs.setString(2,str);
                Vector v = FMSObjCon.addObjectToStatement(3,ed,cs);
                cs.registerOutParameter(4, java.sql.Types.INTEGER);
                cs.execute();
System.err.println("CCEdgeDisplay Object ID: "+cs.getInt(4));
                FMSObjCon.cleanUp(v);
                }
            }
        catch( Exception e ) 
