...
/**
 * CCNetworkActionObserverWindow: Is this window displayed to observers during
 * the running of a CCNetworkAction.
 * <p>
 * <br> Started: 07-24-2001
...
public class CCNetworkActionObserverWindow extends ObserverWindow
    {
    CCNetwork m_network;
    ...
    CCObserverDisplayCanvas m_displayArea;
    CCObserverMiniDisplayCanvas m_miniDisplayArea;
    ...
    public CCNetworkActionObserverWindow(ExptOverlord app1, ObserverExptInfo app2, ExptMessageListener app3)
        {
        ...
        setTitle(m_EOApp.getLabels().getObjectLabel("ccnaow_title"));
        setSize(m_EOApp.getWidth(),m_EOApp.getHeight());
    
        m_displayArea = new CCObserverDisplayCanvas(m_network);
        m_miniDisplayArea = new CCObserverMiniDisplayCanvas(m_network,m_displayArea);
    ...
    // Set up menu bar
        ...
        m_fileMenu = new Menu(m_EOApp.getLabels().getObjectLabel("ccnaow_file"));

        tmpItem = new MenuItem(m_EOApp.getLabels().getObjectLabel("ccnaow_exit"));
    ...
    // North Panel
       ...
       northPanel.constrain(new BorderPanel(tmpPanel,BorderPanel.FRAME),7,1,2,2,GridBagConstraints.CENTER,GridBagConstraints.BOTH);
    ...
    // Start Setup West Panel
        ...
        westPanel.constrain(new Label(m_EOApp.getLabels().getObjectLabel("ccnaow_timer")),1,1,2,1);
        westPanel.constrain(m_timeLabel,3,1,2,1);

        m_roundLabel = new FixedLabel(3,"-");
        westPanel.constrain(new Label(m_EOApp.getLabels().getObjectLabel("ccnaow_round")),1,2,2,1);
        westPanel.constrain(m_roundLabel,3,2,2,1);

        westPanel.constrain(new Label(m_EOApp.getLabels().getObjectLabel("ccnaow_sr")),1,10,4,1);
        m_subjectList = new FixedList(4,false,2,7);
        initializeSubjectList();
        westPanel.constrain(m_subjectList,1,11,4,4);

        m_subjectDetailsButton = new Button(m_EOApp.getLabels().getObjectLabel("ccnaow_details"));
        m_subjectDetailsButton.addActionListener(this);
        westPanel.constrain(m_subjectDetailsButton,1,16,4,1);
        
    // End Setup for West Panel

    // Start Setup East Panel
        GridBagPanel eastPanel = new GridBagPanel();

        eastPanel.constrain(new Label(m_EOApp.getLabels().getObjectLabel("ccnaow_ei")),1,1,4,1);
        m_edgeList = new FixedList(3,false,2,8);
        initializeEdgeList();
        eastPanel.constrain(m_edgeList,1,2,4,4);

        m_edgeDetailsButton = new Button(m_EOApp.getLabels().getObjectLabel("ccnaow_details"));
        m_edgeDetailsButton.addActionListener(this);
        eastPanel.constrain(m_edgeDetailsButton,1,7,4,1);

        m_zoomInButton = new Button(m_EOApp.getLabels().getObjectLabel("ccnaow_zi"));
        m_zoomInButton.addActionListener(this);
        eastPanel.constrain(m_zoomInButton,1,8,4,1);

        m_zoomOutButton = new Button(m_EOApp.getLabels().getObjectLabel("ccnaow_zo"));
        m_zoomOutButton.addActionListener(this);
        eastPanel.constrain(m_zoomOutButton,1,9,4,1);
    ...
    public void initializeLabels()
        {
        m_EOApp.initializeLabels("girard/sc/cc/awt/ccnaow.txt");
        }
    ...
    public void removeLabels()
        {
        m_EOApp.removeLabels("girard/sc/cc/awt/ccnaow.txt");
        }
