...
public class CCClientMiniDisplayCanvas extends Canvas implements MouseListener,MouseMotionListener
    {
    ...
    CCNetwork m_network;
    CCClientDisplayCanvas m_area;
    boolean m_mousePressed = false;

    public CCClientMiniDisplayCanvas(CCNetwork n,CCClientDisplayCanvas edc)
        {
        m_network = n;
        m_area = edc;
    ...
    public CCClientMiniDisplayCanvas(CCNetwork n, CCClientDisplayCanvas edc, Point tl, Point br)
        {
        m_network = n;
        m_topLeft = tl;
        m_bottomRight = br;
    ...
    public void zoomAdjust(int zoomLevel)
        ...
        if ((midX + tmpW) > NetworkBuilder.WIDTH)
            midX = NetworkBuilder.WIDTH - width/2;
        if ((midX - tmpW) < 0)
            midX = width/2;
        
        if ((midY + tmpH) > NetworkBuilder.HEIGHT)
