...
*****************************************************************************/
public class CCPauseResumeWindow extends Frame implements ActionListener
    {
    CCNetworkActionExperimenterWindow m_NAEWApp;
    ExptOverlord m_EOApp;

    Button m_ResumeButton;

    public CCPauseResumeWindow(CCNetworkActionExperimenterWindow app)
        {
        super();
        m_NAEWApp = app;
        ...
        setTitle(m_EOApp.getLabels().getObjectLabel("ccprw_title"));
        setFont(m_EOApp.getMedWinFont());
        setBackground(m_EOApp.getWinBkgColor());

        GridBagPanel MainPanel = new GridBagPanel();

        MainPanel.constrain(new Label(m_EOApp.getLabels().getObjectLabel("ccprw_einp")),1,1,4,1,GridBagConstraints.CENTER);

        MainPanel.constrain(new Label(m_EOApp.getLabels().getObjectLabel("ccprw_pptre")),1,2,4,1,GridBagConstraints.CENTER);
    
        m_ResumeButton = new Button(m_EOApp.getLabels().getObjectLabel("ccprw_resume"));
...
*********************************************************************************/
    ...
    public void initializeLabels()
        {
        m_EOApp.initializeLabels("girard/sc/cc/awt/ccprw.txt");
        }

    public void removeLabels()
        {
        m_EOApp.removeLabels("girard/sc/cc/awt/ccprw.txt");
        }
    }