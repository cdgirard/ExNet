...
*****************************************************************************/
public class BENodeSNWindow extends Frame
    {
    ...
    public BENodeSNWindow(BENetworkActionClientWindow app)
        {
        super();
        m_NACWApp = app;
   ...
   *****************************************************************/
        ...
        setTitle(m_EOApp.getLabels().getObjectLabel("bensnw_title"));
     ...
     // Create color code display.
        ...
        g.drawString(m_EOApp.getLabels().getObjectLabel("bensnw_iac"),2,15);

        g.setColor(BEColor.INCLUSIVE);
        g.fillOval(260,24,12,12);
        g.setColor(BEColor.black);
        g.drawString(m_EOApp.getLabels().getObjectLabel("bensnw_ic"),2,35);

        g.setColor(BEColor.EXCLUSIVE);
        g.fillOval(260,44,12,12);
        g.setColor(BEColor.black);
        g.drawString(m_EOApp.getLabels().getObjectLabel("bensnw_ec"),2,55);

        g.setColor(BEColor.INCLUSIVE_EXCLUSIVE);
        g.fillOval(260,64,12,12);
        g.setColor(BEColor.black);
        g.drawString(m_EOApp.getLabels().getObjectLabel("bensnw_iec"),2,75);

        g.setColor(BEColor.NULL);
        g.fillOval(260,84,12,12);
        g.setColor(BEColor.black);
        g.drawString(m_EOApp.getLabels().getObjectLabel("bensnw_nc"),2,95);
     ...
     // End create color code display.
        ...
        while (enm.hasMoreElements())
            ...
            str.append(m_EOApp.getLabels().getObjectLabel("bensnw_node")+" "+node.getLabel()+" ");
            str.append(m_EOApp.getLabels().getObjectLabel("bensnw_max")+" "+(Integer)data.get("Max")+" ");
            str.append(m_EOApp.getLabels().getObjectLabel("bensnw_min")+" "+(Integer)data.get("Min"));
    ...
    public StringBuffer fillInSubnetworkInfo(Hashtable data,String tabSpace)
        ...
        str.append("\n"+tabSpace+m_EOApp.getLabels().getObjectLabel("bensnw_subnetwork")+" "+(Integer)data.get("Subnetwork")+" ");
        
        str.append(" [ "+m_EOApp.getLabels().getObjectLabel("bensnw_nodes"));
    ...
    public void initializeLabels()
        {
        m_EOApp.initializeLabels("girard/sc/be/awt/bensnw.txt");
        }

    public void removeLabels()
        {
        m_EOApp.removeLabels("girard/sc/be/awt/bensnw.txt");
        }
    }
