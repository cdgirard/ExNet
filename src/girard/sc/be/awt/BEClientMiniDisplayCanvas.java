...
public class BEClientMiniDisplayCanvas extends Canvas implements MouseListener,MouseMotionListener
    {
    Point m_topLeft = new Point(0,0);
    Point m_bottomRight = new Point(NetworkBuilder.WIDTH,NetworkBuilder.HEIGHT);
    double[] m_level = { 0.65, 0.80, 1, 1, 1 };
    BENetwork m_network;
    BEClientDisplayCanvas m_area;
    boolean m_mousePressed = false;

    public BEClientMiniDisplayCanvas(BENetwork n,BEClientDisplayCanvas edc)
        {
        m_network = n;
        m_area = edc;
    ...
    public BEClientMiniDisplayCanvas(BENetwork n, BEClientDisplayCanvas edc, Point tl, Point br)
        {
        m_network = n;
        m_topLeft = tl;
        m_bottomRight = br;
    ...
    public void paint(Graphics g)
        ...
        while (enm.hasMoreElements())
            ...
            String n1type = (String)((BENode)m_network.getNode(bee.getNode1())).getExtraData("Type");
            String n2type = (String)((BENode)m_network.getNode(bee.getNode2())).getExtraData("Type");
     ...
     // Will display all relations connected to me or my neighbors.
            ...
            if (drawFlag)
                ...
                newLoc1.x = (int)(xAdj*bee.getN1Anchor().x);
                newLoc1.y = (int)(yAdj*bee.getN1Anchor().y);

                Point newLoc2 = new Point();
                newLoc2.x = (int)(xAdj*bee.getN2Anchor().x);
                newLoc2.y = (int)(yAdj*bee.getN2Anchor().y);
