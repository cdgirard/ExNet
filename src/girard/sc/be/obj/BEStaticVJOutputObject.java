...
/**
 * This object stores the join votes for static coalitions that are part of
...
public class BEStaticVJOutputObject extends DataOutputObject implements Serializable
    {
    ...
    public BEStaticVJOutputObject ()
        {
        }
    public BEStaticVJOutputObject(int exptID, int ai, int p, int r, int n, int c, boolean j)
        {
        m_exptID = exptID;
        m_actionIndex = ai;
        m_period = p;
    ...
    public BEStaticVJOutputObject(ResultSet rs)
        {
        try
            {
    ...
    // Expt_Out_ID_INT, Action_Index, Period_INT, Round_INT, Node_INT, Coalition_INT, Vote_BIT
        return new String("{call up_insert_JBEExptStaticVJData (?, ?, ?, ?, ?, ?, ?)}");
        }
    public int getNode()
        {
        return m_node;
